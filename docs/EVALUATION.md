...
We evaluate robustness of three DP strategies each trained on different regimes and tested across 997 [Polymarket price paths](https://www.kaggle.com/datasets/sandeepkumarfromin/full-market-data-from-polymarket): 
...
3. **Mean Reverting Regime:** Aggressive Policy - assumes prices revert to fair value

For each of these strategies, we also try varying $p_{subj} \in [0.1,0.2,...,0.9]$ to reflect varying level's of a trader's (possibly misplaced) confidence about an event occuring. This allows us to see which regime is most robust to misspecification of $p_{subj}$.

## Non-DP Baselines
We also test the following baselines:
1. **Buy and Hold**
2. **Greedy 1/4-Kelly**
3. **Greedy 1/4-Kelly including spread**
4. **Buy at 90% $p_{subj}$, sell at 110% $p_{subj}$**

*We use 1/4-Kelly rather than full Kelly for baselines as it is common practice to reduce variance while preserving most growth.*
...
#### **1. Data Imbalance:** 
* Of the 997 Polymarket price paths, more than 90% of YES contracts resolve to 1. This could bias our results to favor high $p_{subj}$. Assuming the events of each price path are all independent of one another; we can flip outcomes randomly such that 50% of YES contracts resolve to 1 and 50% resolve to 0. 
#### **2. Data Dependence:**
...
#### **3. Comparing $p_{subj}$ with binary (0/1) event resolution:**
